...
function initApp() {
    const app = {
        ...
        async initDatabase() {
            ...
            const productsFromLaravel = await response.json();

            // Clear existing products from IndexedDB
            const tx = this.db.db.transaction("products", "readwrite");
            const store = tx.objectStore("products");
            await store.clear();

            // Add Laravel products into IndexedDB
            for (let product of productsFromLaravel) {
                await store.add(product);
            }

            await tx.done; // optional, ensures the transaction completes

            // Now load from IndexedDB into the UI
            this.loadProducts();
        },
